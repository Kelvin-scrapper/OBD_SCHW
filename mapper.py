...
class SchwabDataMapper:
    ...
    def parse_monthly_activity_report(self, text):
        ...
        month_keys = []
        # The PDF has a year-header row with 2 years (e.g. "2025  2026") one line above the
        # month abbreviations.  If we find a line with ≥2 years, take the earlier one.
        # Fallback: title has only the END year (e.g. "For March 2026") → start = end_year - 1.
        year = None
        for context_line in lines[max(0, month_line_idx - 5):month_line_idx + 1]:
            years_in_line = [int(y) for y in re.findall(r'\b(20\d\d)\b', context_line)]
            if len(years_in_line) >= 2:
                year = min(years_in_line)
                break
        if year is None:
            for context_line in lines[max(0, month_line_idx - 5):month_line_idx + 1]:
                years_in_line = [int(y) for y in re.findall(r'\b(20\d\d)\b', context_line)]
                if years_in_line:
                    year = min(years_in_line) - 1  # title has end-year; start is one year earlier
                    break
        if year is None:
        ...
        dat_label_found = False  # DAT label and values may be on separate lines in PyPDF2
        ...
        for i, line in enumerate(lines):
            ...
            if line_clean.startswith('Beginning Client Assets'):
            ...
            elif dat_label_found:
                # Previous line was the DAT label; values are on this line
                values = re.findall(r'[\d,]+', line_clean)
                filtered = [v for v in values if 3000 <= int(v.replace(',', '')) <= 20000]
                if filtered:
                    data_rows['Daily Average Trades (DATs)'] = [float(v.replace(',', '')) for v in filtered[:len(months)]]
                dat_label_found = False

            elif 'Daily Average Trades' in line_clean and 'Derivative' not in line_clean:
                values = re.findall(r'[\d,]+', line_clean)
                # DAT values are 4-5 digit thousands (3,000 – 20,000); skip footnotes & smaller nums
                filtered = [v for v in values if 3000 <= int(v.replace(',', '')) <= 20000]
                if filtered:
                    data_rows['Daily Average Trades (DATs)'] = [float(v.replace(',', '')) for v in filtered[:len(months)]]
                else:
                    # Values may be on the next line (PyPDF2 splits label from values)
                    dat_label_found = True

            elif in_net_buy_sell_section and line_clean.startswith('Mutual Funds'):
                # Handles both old format (millions, e.g. (8,537)) and new (billions, e.g. (8.5))
                # Footnote refs like (7) or (8) are excluded: they have no decimal and <3 digits
                values = re.findall(r'\([\d,]*\.[\d]+\)|\([\d,]{3,}\)|[\d,]*\.[\d]+|[\d,]{4,}', line_clean)
                ...
                values = re.findall(r'\([\d,]*\.[\d]+\)|\([\d,]{3,}\)|[\d,]*\.[\d]+|[\d,]{4,}', line_clean)
                data_rows['Exchange-Traded Funds'] = [self._parse_value(v) for v in values[:len(months)]]
    ...
    def parse_smart_supplement(self, text):
        ...
        # Find the month headers line (track index for year detection)
        month_line = None
        month_line_idx = None
        for idx, line in enumerate(lines):
            if re.search(r'(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)', line):
                month_line = line
                month_line_idx = idx
                break
        ...
        # Detect starting year: prefer a line with 2 years (year-header row),
        # fall back to single year in context minus 1.
        month_to_num = {
            'Jan': 1, 'Feb': 2, 'Mar': 3, 'Apr': 4, 'May': 5, 'Jun': 6,
            'Jul': 7, 'Aug': 8, 'Sep': 9, 'Oct': 10, 'Nov': 11, 'Dec': 12
        }
        start_year = None
        for ctx in lines[max(0, month_line_idx - 5):month_line_idx + 1]:
            yy = [int(y) for y in re.findall(r'\b(20\d\d)\b', ctx)]
            if len(yy) >= 2:
                start_year = min(yy)
                break
        ...
        # Build dynamic month keys using rollover detection
        raw_months = re.findall(r'\b(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\b', month_line)
        all_months = []
        year = start_year
        prev_num = month_to_num[raw_months[0]]
        for m in raw_months:
            num = month_to_num[m]
            if num < prev_num and prev_num - num > 6:
                year += 1
            all_months.append(f"{year}-{num:02d}")
            prev_num = num

        print(f"[+] SMART supplement month keys: {all_months[0]} to {all_months[-1]}")
    ...
    def extract_all_data(self):
        ...
        for month in all_months:
            ...
            # Daily Average Trades — monthly activity report (direct) > press release monthly
            #                       > 13-week report > quarterly earnings average
            month_qkey = f"{month.split('-')[0]}-Q{(int(month.split('-')[1])-1)//3+1}"
            row['USA.OBD.SCHWAB.CLIENTACTIVITY.AVERAGETREDS.M'] = (
                data.get('Daily Average Trades (DATs)')
                or data.get('Average Daily Trades')
